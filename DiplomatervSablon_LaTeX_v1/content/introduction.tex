...
\begin{enumerate}
	\item \emph{Feladatspecifikáció: (\refstruc{sec:Sepcification})} 
        \begin{enumerate}
            ...
            \item \emph{Diagramok: (\refstruc{sec:SpecificationDiagrams})} Bemutatok néhány diagramot és ábrát, amelyek segítik az alkalmazás felépítésének megértését.
        \end{enumerate}
	\item \emph{Irodalomkutatás: (\refstruc{sec:Search})} 
        ...
        \end{enumerate}
    \item \emph{Felsőszintű architektúra: (\refstruc{sec:Architecture})} Nagyvonalakban bemuattom az alakalmazás fő vázát és felépítéséét.
        \begin{enumerate}
        ...
        \end{enumerate}
    \item \emph{Részletes megvalósítás: (\refstruc{sec:Details})} Kitérek minden az eddigiekben bemutatott technológia pontos használatára, és arra, hogy nálam milyen formában jelentek meg.
        \begin{enumerate}
            \item \emph{Compose: (\refstruc{sec:Compose})} 
            \item \emph{Navigáció: (\refstruc{sec:Nav})}
            \item \emph{ViewModel: (\refstruc{sec:VM})}
            \item \emph{HTTP kliens: (\refstruc{sec:HTTP})}
            \item \emph{PDF: (\refstruc{sec:PDF})}
            \item \emph{Szövegfelismerés: (\refstruc{sec:TextRec})}
        \end{enumerate}
    \item \emph{Továbbfejlesztési lehetőségek: (\refstruc{sec:Future})} 
    \item \emph{Összefoglalás: (\refstruc{sec:Summary})} 
\end{enumerate}
